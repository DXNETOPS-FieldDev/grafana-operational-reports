# Top-N Devices & Models with Most Alarms

*Dashboard file: `spectrum-alarm-top-devices.json` · Folder: Alarm Reports · [Deploy guide](../Deploying-to-a-New-Grafana-Environment.md)*

Not linked to or from any other dashboard in this set, except one outbound link — it drills down into **[Device Detail](spectrum-device-detail.md)** (imported separately — see that page).

## What this shows

Ranks devices and models by alarm volume: totals tiles plus a ranked bar chart/table showing % of total and frequency per day. A prioritization list for the flakiest hardware — replacement, firmware, or config candidates.

## How to import

1. Follow the **[Deploy guide](../Deploying-to-a-New-Grafana-Environment.md)** for the shared mechanics (folders, service-account token, deploy script vs. manual import). This page only covers what's specific to Top-N Devices & Models with Most Alarms.
2. Import `spectrum-alarm-top-devices.json`, plus `spectrum-device-detail.json` (see **[Device Detail](spectrum-device-detail.md)**) — its drill-down links need it.
...
| `topN` | No — optional filter | 20 | How many devices/models to rank. |
